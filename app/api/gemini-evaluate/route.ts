...
export async function POST(req: NextRequest) {
  let { script, question, answer } = await req.json();
  script = script ?? "";
  question = question ?? "";
  answer = answer ?? "";
  if (!script.trim() || !question.trim() || !answer.trim()) {
    return NextResponse.json({ error: "script, question, answer 모두 필요합니다." }, { status: 400 });
  }
